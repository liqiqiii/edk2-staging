  ...
  )
{
  ...
  while (ExtendedCapPtr != 0) {
    ...
    if ((UINT16)ExtendedCapHeader == PCI_EXPRESS_EXTENDED_CAPABILITY_DOE_ID) {
      DEBUG((DEBUG_ERROR, "[LocatePcieDoeCapStructure] Found PCIe DOE Cap structure ...\n"));
      ...
      if (!EFI_ERROR(Status)) {
        DEBUG((DEBUG_ERROR, "[LocatePcieDoeCapStructure] PCIe DOE Cap structure is located for step 0 before interuppt support. Offset = 0x%x\n", ExtendedCapPtr));
        // Check DOE version and capabilities
        // temporarily disable as the NVMe I'm using doesn't have this support
